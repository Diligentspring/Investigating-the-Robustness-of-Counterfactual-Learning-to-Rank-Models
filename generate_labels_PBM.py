from click_simulator import PositionBiasedModel
import sys
import random
...
if eta == 0.5:
    gen_label_path = 'PBM_eta0d5.labels'
else:
    gen_label_path = 'PBM_eta{}.labels'.format(int(eta))
gen_label_file = open(gen_label_path, 'w')
# gen_label_file = open('t'+ train_pl_path + '/' + gen_label_path, 'w')
# gen_label_file = open('PBM_eta0d5.labels', 'w')
...
for line in initlist_file:
    if line != '':
        ...
        click_list, exam_p_list, click_p_list = PBM.sampleClicksForOneList(labels)
        for c in click_list:
