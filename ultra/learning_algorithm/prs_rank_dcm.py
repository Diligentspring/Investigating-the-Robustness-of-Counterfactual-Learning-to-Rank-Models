...
class PRSrank_DCM(BaseAlgorithm):
    ...
    def __init__(self, data_set, exp_settings):
        ...
        pw_file = open('./{}/pw/{}_cont.txt'.format(model_dir, train_data_prefix), 'r')

        conts = pw_file.readline().strip('\n').split(' ')
        # conts = [max(float(cont), 0.01) for cont in conts]
        conts = [float(cont) for cont in conts]
        self.conts = conts[:self.rank_list_size]
    ...
    def train(self, input_feed):
        ...
        const_one = torch.tensor(1).to(device=self.cuda)
        for i in range(len(labels)):
            ...
            for j in range(self.rank_list_size):
                if cont_index == -1:
                    ...
                    self.propensity[i][j] = self.conts[cont_index]
                if int(labels[i][j].data) == 1:
                    cont_index = j

        ipw = torch.ones_like(self.propensity).to(device=self.cuda) / self.propensity
