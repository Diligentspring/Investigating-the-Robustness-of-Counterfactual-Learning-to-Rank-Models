...
class BaseAlgorithm(ABC):
    ...
    def remove_padding_for_metric_eval(self, input_id_list, model_output):
        ...
        valid_flags = torch.cat(
            (torch.ones(
                self.letor_features.shape[0]), torch.zeros(
                [1])), dim=0)
        ...
        for i in range(len(output_scores)):
            if self.is_cuda_avail:
                output_scores[i] = torch.where(
                    ...
                    torch.ones_like(
                        output_scores[i],
                        device=self.cuda) * self.PADDING_SCORE
                )
            else:
                output_scores[i] = torch.where(
                    input_flag_list[i],
    ...
    def get_ranking_scores(self, model, input_id_list, **kwargs):
        ...
        letor_features = np.concatenate(
            (self.letor_features, PAD_embed), axis=0)
        input_feature_list = []
    ...
    def create_input_feed(self, input_feed, list_size):
        ...
        self.docid_inputs = torch.as_tensor(
            data=np.array(self.docid_inputs), dtype=torch.int64)
    ...
    def pairwise_loss_on_list(self, output, labels,
        ...
        for i in range(len(sliced_output)):
            for j in range(i + 1, len(sliced_output)):
                ...
                # cur_pair_loss = - \
                #     torch.exp(
                # sliced_output[i]) / (torch.exp(sliced_output[i]) +
                # torch.exp(sliced_output[j])) #这样loss很可能是负数
                cur_pair_loss = torch.log(
                    1 + torch.exp(-cur_label_weight * (sliced_output[i] - sliced_output[j])))
