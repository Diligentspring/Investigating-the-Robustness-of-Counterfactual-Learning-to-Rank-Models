...
class DBGD(BaseAlgorithm):
    ...
    * Yisong Yue and Thorsten Joachims. 2009. Interactively optimizing information retrieval systems as a dueling bandits problem. In ICML. 1201–1208.
    ...
    def train(self, input_feed):
        ...
        with torch.no_grad():
            self.output = self.ranking_model(
                self.model, self.max_candidate_num)
        ...
        if self.hparams.need_interleave:
            ...
            self.winners = self.click_simulation_winners(
                input_feed, self.output, self.interleaving_strategy)
            ...
            if isinstance(layer, nn.Sequential):
                for name, parameter in layer.named_parameters():
                    if "linear" in name:
                        ...
                        for i in range(gradient_matrix.dim() -
                                       expended_winners.dim()):
        ...
        for layer in self.model.children():
            if isinstance(layer, nn.Sequential):
                for name, param in layer.named_parameters():
                    if "linear" in name:
                        noisy_params[name] = F.normalize(torch.normal(
                            mean=0.0, std=1.0, size=param.shape), dim=0)
        return noisy_params
    ...
    def click_simulation_winners(
            self, input_feed, rank_scores, interleave_strategy):
        ...
        for i in range(local_batch_size):
            ...
            while valid_idx > -1:
                if input_feed[self.docid_inputs_name[valid_idx]
                              ][i] < letor_features_length:  # a valid doc
                    break
                valid_idx -= 1
            ...
            input_feed[self.winners_name][i] = self.interleaving.infer_winner(
                click_list)
