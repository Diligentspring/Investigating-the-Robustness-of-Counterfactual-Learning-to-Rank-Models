...
class IPS_PBM_regression(BaseAlgorithm):
    ...
    * Thorsten Joachims, Adith Swaminathan, Tobias Schnahel. 2017. Unbiased Learning-to-Rank with Biased Feedback. In Proceedings of WSDM '17
    ...
    def __init__(self, data_set, exp_settings):
        ...
        # pw_file = open('./{}/pw/{}_exam.txt'.format(model_dir, train_data_prefix), 'r')  # IPS-PBM-EM
        pw_file = open('./{}/pw/{}_exam_regression.txt'.format(model_dir, train_data_prefix), 'r')  # IPS-PBM-Regression-EM
        # pw_file = open('./{}/oracle_pw/{}_exam.txt'.format(model_dir, train_data_prefix), 'r') # IPS-PBM-oracle
        # pw_file = open('./{}/dla_pw/{}_exam.txt'.format(model_dir, train_data_prefix), 'r')  # IPS-PBM-dla

        exams = pw_file.readline().strip().split(' ')
        exams = [float(exam) for exam in exams]
        if self.rank_list_size > len(exams):
            for _ in range(self.rank_list_size - len(exams)):
                exams.append(exams[-1])
        self.exams = exams[:self.rank_list_size]
        pw_file.close()
    ...
    def train(self, input_feed):
        ...
        for i in range(len(input_feed[self.labels_name[0]])):
            ...
            pw_list = [1.0 / float(exam) for exam in self.exams]
