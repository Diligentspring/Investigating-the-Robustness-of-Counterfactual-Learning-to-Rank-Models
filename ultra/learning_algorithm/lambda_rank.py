...
class LambdaRank(BaseAlgorithm):
    ...
    def __init__(self, data_set, exp_settings):
        ...
        self.device = torch.device(
            'cuda') if self.is_cuda_avail else torch.device('cpu')
        ...
        if 'selection_bias_cutoff' in self.exp_settings:
            ...
            self.t_plus = torch.ones(
                [1, self.rank_list_size], device=self.device)
    ...
    def train(self, input_feed):
        ...
        pair_loss_debias = ultra.utils.metrics._safe_div(
            pair_loss, t_plus_t_minus)
        ...
        #     self.t_minus = (1 - self.hparams.EM_step_size) * self.t_minus + self.hparams.EM_step_size * torch.pow(
        #         torch.cat(
        # t_minus_loss_list, dim=1) / t_minus_loss_list[0], 1 /
