...
class PRSrank(BaseAlgorithm):
    ...
    def __init__(self, data_set, exp_settings):
        ...
        self.propensity_file_path = self.exp_settings['propensity_file_path']
    ...
    def train(self, input_feed):
        ...
        ipw = torch.stack(ipw)
        pw = ultra.utils.metrics._safe_div(
            torch.tensor(1.0).to(device=self.cuda), ipw)
        ...
        training_output = self.ranking_model(self.model,
                                             self.rank_list_size)
        # training_output = torch.nan_to_num(training_output_raw)  # the output
        # of the ranking model may contain nan

        preds_sorted, preds_sorted_inds = torch.sort(
            training_output, dim=1, descending=True)
        labels_sorted_via_preds = torch.gather(
            self.labels, dim=1, index=preds_sorted_inds)
        ipw_sorted_via_preds = torch.gather(
            ipw, dim=1, index=preds_sorted_inds)
        # print(ipw_sorted_via_preds[0])
        pw_sorted_via_preds = torch.gather(pw, dim=1, index=preds_sorted_inds)
        # print(pw_sorted_via_preds[0])

        # calculate the prs score using the pw of unclick document and ipw of
        # clicked document
        prs = torch.unsqueeze(ipw_sorted_via_preds, dim=2) * \
            torch.unsqueeze(pw_sorted_via_preds, dim=1)
        ...
        s_ij = torch.unsqueeze(preds_sorted, dim=2) - \
            torch.unsqueeze(preds_sorted, dim=1)
        p_ij = 1.0 / (torch.exp(-self.sigma * s_ij) + 1.0)
        ideally_sorted_labels, _ = torch.sort(
            self.labels, dim=1, descending=True)
    ...
    def dcg(self, labels):
        ...
        numerator = torch.pow(
            torch.tensor(
                2.0, device=self.device), labels.to(
                torch.float32)) - 1.0
    ...
    def compute_delta_ndcg(self, ideally_sorted_stds, stds_sorted_via_preds):
        ...
        batch_ng_diffs = torch.unsqueeze(
            batch_n_gains, dim=2) - torch.unsqueeze(batch_n_gains, dim=1)
        ...
        # discount co-efficients
        batch_dists = 1.0 / torch.log2(batch_std_ranks + 2.0)
        batch_dists = torch.unsqueeze(batch_dists, dim=0)
        batch_dists_diffs = torch.unsqueeze(
            batch_dists, dim=2) - torch.unsqueeze(batch_dists, dim=1)
