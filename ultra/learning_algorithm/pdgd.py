    ...
    def train(self, input_feed):
        ...
        for i in range(len(input_feed[self.labels_name[0]])):
            for j in range(self.rank_list_size):
                # not a valid doc
                if input_feed[self.docid_inputs_name[j]
                              ][i] == letor_features_length:
        ...
        for i in range(len(input_feed[self.labels_name[0]])):
            ...
            for j in range(self.rank_list_size):
                ...
                # not a valid doc
                if input_feed[self.docid_inputs_name[l]
                              ][i] == letor_features_length:
                    continue
                if input_feed[self.labels_name[l]][i] > 0:  # a clicked doc
                    for k in range(l + 2):
                        ...
                        if k < self.rank_list_size and \
                            ...
                            # not a valid doc
                            if input_feed[self.docid_inputs_name[k]
                                          ][i] == letor_features_length:
                                continue
        ...
        self.positive_docid_inputs = torch.as_tensor(
            data=positive_docids, dtype=torch.int64)
        self.negative_docid_inputs = torch.as_tensor(
            data=negative_docids, dtype=torch.int64)
        if self.is_cuda_avail:
            self.pair_weights = torch.as_tensor(
                data=pair_weights, device=self.cuda)
        else:
