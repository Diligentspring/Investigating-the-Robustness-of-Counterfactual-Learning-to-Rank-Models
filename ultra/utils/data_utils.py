...
class Raw_data:
    def __init__(self, data_path=None, file_prefix=None,
                 click_model_dir=None, rank_cut=None):
        """
                ...
                if x != '':
                    ...
                    if feautre_idx < self.feature_size:
                        if float(arr2[1]) < 1.7e+308:
                            self.features[arr[0]][int(
                                feautre_idx)] = float(arr2[1])
        ...
        for line in init_list_fin:
            ...
            if rank_cut:
                self.initial_list.append(
                    [x for x in arr[1].strip().split(' ')[:rank_cut]])
            else:
                self.initial_list.append(
                    [x for x in arr[1].strip().split(' ')])
    ...
    def pad(self, rank_list_size, pad_tails=True):
        ...
        # vector for pad
        self.features[-1] = [0.0 for _ in range(self.feature_size)]
